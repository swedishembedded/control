...
class CheckPatch(ComplianceTest):
    ...
    def run(self):
        ...
        diff = subprocess.Popen(
            ("git", "diff", COMMIT_RANGE), stdout=subprocess.PIPE, cwd=GIT_TOP
        )
        try:
            subprocess.run(
...
class KconfigCheck(ComplianceTest):
    ...
    def get_kconfig_dts(self, kconfig_dts_file):
        ...
        # Invoke the script directly using the Python executable since this is
        # not a module nor a pip-installed Python utility
        zephyr_drv_kconfig_path = os.path.join(
            ZEPHYR_BASE, "scripts", "dts", "gen_driver_kconfig_dts.py"
        )
...
"""
    ...
    def check_no_undef_outside_kconfig(self, kconf):
        ...
        for grep_line in grep_stdout.splitlines():
            ...
            for sym_name in re.findall(regex, line):
                ...
                if (
                    ...
                    and sym_name not in self.UNDEF_KCONFIG_WHITELIST
                ):
        ...
        undef_desc = "\n".join(
            f"CONFIG_{sym_name:35} {', '.join(locs)}"
    ...
    def ls_owned_files(self, codeowners):
        ...
        with open(codeowners, "r") as codeo:
            for lineno, line in enumerate(codeo, start=1):
                ...
                if not files:
                    self.failure(
                        f"Path '{git_patrn}' not found in the tree"
...
concatenated together, so no document separators are needed."""
    ...
    def check_source_file(self, fname):
        ...
        if not contents.endswith("\n"):
            self.failure(
                f"Missing newline at end of '{fname}'. Check your text "
                f"editor settings."
            )
        ...
        try:
            ...
            for m in matches:
                ...
                if m[3][0] in ("F", "E"):
                    ...
                    severity = "warning"
                self.fmtd_failure(severity, m[3], m[0], m[1], col=m[2], desc=m[4])
...
class Identity(ComplianceTest):
    ...
    def run(self):
        for shaidx in get_shas(COMMIT_RANGE):
            ...
            error1 = (
                f"{sha}: author email ({author}) needs to match one of "
                f"the signed-off-by entries."
            )
            error2 = (
                f"{sha}: author email ({author}) does not follow the "
                f"syntax: First Last <email>."
            )
            error3 = (
                f"{sha}: author email ({author}) must be a real email "
...
class BinaryFiles(ComplianceTest):
    ...
    def run(self):
        ...
        for stat in git(
            "diff", "--numstat", "--diff-filter=A", COMMIT_RANGE
        ).splitlines():
