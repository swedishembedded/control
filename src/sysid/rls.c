...
static void recursive(uint8_t NP, uint8_t NZ, uint8_t NZE, float y, float phi[], float theta[],
		      float P[], float *past_e, float forgetting);
	 ...
	 float forgetting)
{
	...
	if (*count == 0) {
		...
		memset(P, 0,
		       (NP + NZ + NZE) * (NP + NZ + NZE) * sizeof(float)); // Initial P with zeros
