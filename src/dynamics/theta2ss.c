	      ...
	      uint8_t NP, uint8_t NZ, uint8_t NZE, bool integral_action)
{
	...
	if (integral_action == true) {
		...
		for (uint8_t i = 0; i < ADIM - 2; i++)
			A[i * ADIM + 1 + i] =
				1; // Ones at the diagonal with 1 in shift to right to third last row
		A[ADIM * ADIM - 1] =
			1; // This adds 1 at the bottom corner to right  [A 0; CA ->1<-]
		for (uint8_t i = 0; i < NP; i++)
			A[NP * ADIM + i] =
				A[i]; // This is exactly the same as C*A because C = [1 0] and C*A is the first row of A
		...
		for (uint8_t i = 0; i < ADIM - 1; i++)
			A[i * ADIM + 1 + i] =
				1; // Ones at the diagonal with 1 in shift to right to last row
